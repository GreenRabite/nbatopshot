...
const runFunction = async () => {
  ...
  const results =await BB.map(URLS, async url => {
    return axios.get(url)
      .then(response => {
        ...
        return countNumberOfDunks(plays);
      })
      .catch(function (error) {
        // handle error
        // console.log(error)
  ...
  const {results: boxScores, remainingGames: thursdayRemainingGames} = await fetchGameResults(THURSDAY_BOXSCORE_URLS)
  ...
  // pass dunks stats to right player
  allResults.forEach(playByPlayPlayer => {
    allBoxScoresLookup[playByPlayPlayer.playerId] = {
      ...allBoxScoresLookup[playByPlayPlayer.playerId],
      dunks: playByPlayPlayer.dunks
    }
  })

  
  const thursdayPlayers = Object.values(allBoxScoresLookup)
  let thursdayPlayersByGames = {}

  thursdayPlayers.forEach(player => {
    if(thursdayPlayersByGames[player.teams]){
      thursdayPlayersByGames[player.teams].push(player)
    }else{
      thursdayPlayersByGames[player.teams] = [player]
    }
  })
